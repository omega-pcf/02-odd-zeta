\section{Definitions: the PCF ring and torus}\label{sec:background}
%%% ====================================================================
...
\begin{remark}[Canonical identifications]\label{rmk:standard-id}
  ...
  \begin{enumerate}
    ...
    \item every positive integer factors uniquely into primes
          (Fundamental Theorem of Arithmetic; Euclid, \cite[Prop.~IX.14]{heath1956thirteen});
...
%%% ====================================================================
\subsection{Uniqueness of \texorpdfstring{$\varphi$}{phi} and \texorpdfstring{$\mathbb{Q}(\sqrt{5})$}{Q(sqrt(5))}}\label{sec:unique}
%%% ====================================================================
...
%%% ====================================================================
\subsection{Frobenius lifts and the splitting criterion}\label{sec:frobenius-split}
%%% ====================================================================
...
\begin{proof}
  ...
  Representative values for $3 \leq p \leq 59$ are provided in \cref{tab:fib-split}.
\end{proof}
...
%%% ====================================================================
\subsection{Mersenne bridge and localization at \texorpdfstring{$2$}{2}}%
\label{sec:rpcf-role}
%%% ====================================================================
...
\begin{theorem}[Mersenne bridge; González García et al.~\cite{V11}]%
  \label{thm:mersenne}
...
\begin{remark}[The non-circularity chain]\label{rmk:chain}
  ...
  \begin{equation*}
    \begin{split}
      ...
      \;\to\;
      \underbrace{\chi_5(p) = F\_p\bmod p}_{\text{Artin char.}}
      \;\to\;
      \underbrace{G(p\bmod 20)=(\chi_4(p),\chi\_5(p))}_{\text{class functor}} \\[6pt]
...
% ------------------------------------------------------------------
\subsection{The classification group
\texorpdfstring{$\Ztwenty$}{(Z/20Z)*}}\label{sec:Z20}
% ------------------------------------------------------------------

The group $\Ztwenty = \{1,3,7,9,11,13,17,19\}$ of
order $8$ classifies every prime $p > 5$ by its residue
class modulo $20$ (\cref{prop:fib-split}).  The arithmetic
of this group has two key properties:

\begin{proposition}[Multiplicative closure and additive destruction]
\label{prop:Z20-arithmetic}
\begin{enumerate}
\item[\textup{(i)}] $\Ztwenty$ is closed under multiplication:
  for all $a, b \in \Ztwenty$, the product $ab \bmod 20 \in \Ztwenty$.
\item[\textup{(ii)}] $\Ztwenty$ is destroyed under addition:
  for all $a, b \in \Ztwenty$, the sum $a + b \bmod 20 \notin \Ztwenty$.
\end{enumerate}
\end{proposition}

\begin{proof}
Both statements are decidable by exhaustive check over the
$64 = 8 \times 8$ pairs (Lean: \lean{mul\_closed} and
\lean{add\_destroyed}, both verified by \texttt{decide};
cf.~Proposition~2 of~\cite{V11}).
Intuition for (ii): every element of $\Ztwenty$ is odd, so every
sum $a + b$ is even, hence not coprime to $20$, hence not in
$\Ztwenty$.
\end{proof}
