...
(Section~\ref{sec:fundamental}), the period $T = 2\pi\log\golden$ shows that
...
The factorisation~\eqref{eq:main-factorization} uses the specific field
...
\item The Galois conjugation $\golden\leftrightarrow\bar\golden = -1/\golden$
  ...
  \emph{(Proposition~\ref{prop:phi-unique})}.
\end{enumerate}
...
\begin{proof}
$\golden^p = F_p\golden + F_{p-1}$ (Definition~\ref{def:frobenius}).
...
Verified for all primes $3\leq p\leq 59$ (Table~\ref{tab:fib-split}).
\qed
\end{proof}
...
Proposition~\ref{prop:fib-split} expresses the Artin character
...
factor $f_p(s)$ of $\ZKsqrt(s)$---is encoded in the Fibonacci
sequence itself.
\end{remark}
...
$\ZKsqrt(s)$ is $(1-2^{-2s})^{-1}$, not $(1-2^{-s})^{-2}$.

Localising at~$2$ to form $\Rpcf = \mathbb{Z}[\golden,\golden^{-1},\tfrac{1}{2}]$
is justified by the Mersenne bridge~\eqref{eq:mersenne}: $\golden^{\lambda_{\log}}=2$
...
factor follows from Proposition~\ref{prop:local-factors}.
The Mersenne bridge is Theorem~\ref{thm:mersenne}.
\qed
\end{proof}
...
\begin{split}
  ...
  \underbrace{\ZKsqrt(s)=\textstyle\prod_p f_p(s)}_{\text{Euler product}}
  \;\to\;
  \zeta(s).
\end{split}
\end{equation*}
At no point does $\zeta(s)$ itself enter the construction of $\chi_5$
or $\ZKsqrt$; the chain is acyclic.  The localisation at~$2$ is
justified by $\golden^{\lambda_{\log}}=2$ (Theorem~\ref{thm:mersenne}).
Each arrow in this chain is a functor or natural transformation
in the categorical framework of Section~\ref{sec:categorical-framework}:
$\golden^2=\golden+1$ generates the Frobenius functor $\Psi$
(Definition~\ref{def:frobenius-functor}), $\Psi$ determines $\chi_5$
via the Fibonacci splitting criterion
(Proposition~\ref{prop:fib-split}), $\chi_5$ is a component of
$G$ (Theorem~\ref{thm:bridge3}), and $G$ assembles
