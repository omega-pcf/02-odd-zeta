\section{Prime Decomposition and Local Euler Factors}\label{sec:primes}
%%% ====================================================================
...
(Remark~\ref{rmk:standard-id}).  The Dirichlet coefficients
...
The factorisation $\zeta(s) = \ZKsqrt(s)/L(s,\chi_5)$ is not specific
...
%%% ====================================================================
\section{The Fundamental Formula}\label{sec:fundamental}
%%% ====================================================================
...
The formula~\eqref{eq:L1} exposes the role of the circle.
...
%%% ====================================================================
\section{Even Values}\label{sec:even}
%%% ====================================================================
...
\begin{proof}
Standard: generalised Bernoulli formula for primitive even characters; see
...
%%% ====================================================================
\section{The Pentagonal Source}\label{sec:pentagon}
%%% ====================================================================
...
\begin{proposition}[Pentagon identity]\label{prop:pentagon}
\begin{equation}\label{eq:pentagon-phi}
  2\cos\!\tfrac{\pi}{5} = \golden,
  \qquad
  2\cos\!\tfrac{2\pi}{5} = \golden^{-1}.
\end{equation}
More generally, the four pentagonal cosines are:
...
ratio $\golden$ (Euclid, Prop.~XIII.8).  The half-angle of the central
subtended by one side is $\pi/5$, giving $2\cos(\pi/5)=\golden$.
The second identity follows from $\golden^{-1}=\golden-1$ and the
double-angle formula.  Verified at 50-digit precision:
...
combined with Proposition~\ref{prop:pentagon}.
Verified at 50 digits: error $< 10^{-51}$.
\end{proof}

\begin{corollary}[Derivation of $L(1,\chi_5)$ from pentagon]\label{cor:L1}
The class-number formula (Theorem~\ref{thm:L1}) follows from
Proposition~\ref{prop:gauss-log} via the Gauss--Lerch formula for
...
\begin{equation}\label{eq:gauss-lerch}
  ...
  = \frac{T}{\pi\sqrt{5}}.
\end{equation}
The pentagon generates $\golden$ (Proposition~\ref{prop:pentagon}),
...
logarithmic formula of Proposition~\ref{prop:gauss-log}.
Verified at 50 digits: error $< 10^{-50}$.
\end{proof}
...
\begin{proof}
By Proposition~\ref{prop:pentagon}:
...
Verified at 50 digits: error $<10^{-44}$ for all four values.
\qed
\end{proof}

\begin{remark}[Connection to the Galois functor]\label{rmk:pentagon-G}
The pentagonal cosines of Proposition~\ref{prop:pentagon-dictates}
reappear as the $(0,1)$-component of the Galois functor
$G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
(Theorem~\ref{thm:bridge3} below): $\chi_5(a)$ is simultaneously
...
Proposition~\ref{prop:pentagon-dictates} makes explicit what
...
\begin{proof}
Equation~\eqref{eq:hurwitz-rep} is standard (Apostol, Thm.~12.6).
The kernel at $s=1$ follows from $\zeta(s,x) = 1/(s-1) + \psi(x)+O(s-1)$
combined with $\sum\chi_5(a)=0$ (so the $1/(s-1)$ pole and $\log 5$
cancel), giving Proposition~\ref{prop:digamma} and hence
Corollary~\ref{cor:L1}.  The kernel at even $s$ gives
Theorem~\ref{thm:even}.  The odd kernel~\eqref{eq:kernel-odd} is the
direct Hurwitz series, convergent for $s>1$.
\end{proof}

\begin{theorem}[Pentagonal determination of odd zeta values]%
  \label{thm:pentagon-odd}
For $k\geq 0$, the factorisation $\zeta(2k+1) = \ZKsqrt(2k+1)/L(2k+1,\chi_5)$
...
$\ZKsqrt(2k+1)$ are both projections of the same pentagonal Hurwitz
...
\begin{proof}
Items (1)--(3) are Propositions~\ref{prop:pentagon},
\ref{prop:gauss-log}, and Theorem~\ref{thm:bridge3} respectively.
The factorisation is Theorem~\ref{thm:factorisation}.
The Hurwitz representation is Theorem~\ref{thm:hurwitz}.
The $k=0$ closed form is Corollary~\ref{cor:L1}.
For $k\geq 1$: both $\ZKsqrt(2k+1) = \zeta(2k+1)\cdot L(2k+1,\chi_5)$
and $L(2k+1,\chi_5) = 5^{-(2k+1)}\sum\chi_5(a)\zeta(2k+1,a/5)$
are determined by the Hurwitz series at pentagonal points weighted
by $\chi_5$.  Neither factor has a simpler closed form (Open
Problem~\ref{op:closed-form}), but their ratio $\zeta(2k+1)$ is
...
Theorem~\ref{thm:pentagon-odd} establishes \emph{structural} determination: the
...
% ------------------------------------------------------------------
\subsection{Worked example: $\zeta(3)$}\label{ssec:zeta3-example}
% ------------------------------------------------------------------
...
$\ZKsqrt(3) = \prod_p f_p(3)$ and
$L(3,\chi_5) = \prod_p(1-\chi_5(p)p^{-3})^{-1}$.
Their ratio gives
\[
  \zeta(3) = \frac{\ZKsqrt(3)}{L(3,\chi_5)} = 1.2020569\ldots
\]
Every factor in both products is determined by $\chi_5(p)$,
which is determined by $F_p\bmod p$
(Proposition~\ref{prop:fib-split}), which is determined
by $\golden^p = F_p\golden + F_{p-1}$
(Definition~\ref{def:frobenius}).
No parameter is chosen; the pentagon dictates the value.
Verified at 50-digit precision
(\texttt{verify\_odd\_zeta\_pcf\_unified.py}).

The mechanism extends to all odd values.  Table~\ref{tab:zeta-values}
displays the decomposition $\zeta(2k+1) = \ZKsqrt(2k+1)/L(2k+1,\chi_5)$
...
$\ZKsqrt(s)/L(s,\chi_5)$, both factors determined by
$\chi_5$ and hence by $\golden$.  All entries verified at
50-digit precision.}
\label{tab:zeta-values}
\begin{tabular}{rcccc}
\toprule
$s$ & $\ZKsqrt(s)$ & $L(s,\chi_5)$ & $\zeta(s)$ & Source \\
\midrule
\rowcolor[gray]{0.93}
...
%%% ====================================================================
\section{The Period Bridge}\label{sec:period}
%%% ====================================================================
...
The formula $L(1,\chi_5) = T/(\pi\sqrt{5})$ is the bridge: $\pi$ (circle)
and $\log\golden$ (Frobenius regulator) combine in $T$, then $\pi$ cancels
to leave $2\log\golden/\sqrt{5}$.  At $s=2k$ the $\pi$ reappears
explicitly via~\eqref{eq:Lchi-even}.  At $s=2k+1$ for $k\geq 1$ the
...
% ------------------------------------------------------------------
\subsection{Dimensional emergence}\label{ssec:dim-emergence}
% ------------------------------------------------------------------

\begin{proposition}[Dimensional chain from self-measurement]%
  \label{prop:dim-chain}
The two algebraic relations $i^2 = -1$ and $\golden^2 = \golden + 1$
...
is the golden extension of\/ $\mathbb{C}$.  Explicitly:
\begin{enumerate}
\item[\textup{(i)}] \textbf{$\mathbb{R}\to\mathbb{C}$:}
  $i^2 = -1$ means that squaring a 1D generator produces
  a result outside\/ $\mathbb{R}$; the discrepancy is the
  imaginary axis.
\item[\textup{(ii)}] \textbf{$\mathbb{C}\to E^3$:}
  The coupling $z = \golden y$ embeds $\mathbb{C}\cong\mathbb{R}^2$
  into the ambient $\mathbb{R}^3$ by slaving the third coordinate
  to the second: $E^3 = \{(x,\,y,\,\golden y) : x,y\in\mathbb{R}\}
  = \mathrm{span}_{\mathbb{R}}\{e_1,\; e_2+\golden\,e_3\}$.
  The third ambient dimension is present but coupled;
  the irrationality of $\golden$
  (Proposition~\ref{prop:phi-unique}) ensures the coupling
  is non-degenerate.
\item[\textup{(iii)}] \textbf{Self-measurement:}
  Setting $z = \golden y$, the relation $\golden^2 = \golden+1$ gives
  \begin{equation}\label{eq:self-measurement}
    z^2 = \golden^2 y^2 = (\golden+1)y^2 = z\cdot y + y^2.
  \end{equation}
  The square of the third coordinate (a 2D operation applied to
  the new dimension) folds back onto a linear combination of
  lower-dimensional terms $z\cdot y$ and $y^2$.  This is the
  geometric content of $\golden^2 = \golden + 1$: the new
  dimension, when measured against itself, refers back to the
  dimensions that generated it.
\end{enumerate}
\end{proposition}

\begin{proof}
(i) is the standard construction of\/ $\mathbb{C}$ as
$\mathbb{R}[x]/(x^2+1)$.

(ii) The ambient space $\mathbb{R}^3$ has standard basis
...
ambient dimension exists but is slaved to the second by
$\golden^2 = \golden + 1$.  The dimensional emergence is the passage
from $\mathbb{C}\cong\mathrm{span}\{e_1,e_2\}$ (no third coordinate)
to $E^3$ (third coordinate present but coupled).
The irrationality of $\golden$ ensures the coupling is non-degenerate:
$\golden\notin\mathbb{Q}$ means $z = \golden y$ cannot be reduced to a
rational relation between coordinates.

(iii) Direct computation:
$z^2 = (\golden y)^2 = \golden^2 y^2 = (\golden+1)y^2
= \golden y\cdot y + y^2 = zy + y^2$.
\qed
\end{proof}
...
(Proposition~\ref{prop:fib-split}) records the outcome:
\begin{itemize}
...
\end{itemize}
The Euler product $\ZKsqrt(s) = \prod_p f_p(s)$
(Proposition~\ref{prop:local-factors}) is the collective
...
Under the identification of Proposition~\ref{prop:dim-chain} and
Remark~\ref{rmk:frobenius-selfmeas}, the factorisation
\[
  \zeta(s) = \frac{\ZKsqrt(s)}{L(s,\chi_5)}
\]
(Theorem~\ref{thm:factorisation}) admits a reading as dimensional
projection: the one-dimensional $\zeta(s)$ (primes over
$\mathbb{Q}$) is the quotient of the two-dimensional $\ZKsqrt(s)$
...
(Theorem~\ref{thm:pentagon-odd}) are the mixed terms where the
...
(Proposition~\ref{prop:regulator-unifier}) measures the
