\section{Results: the arithmetic formulation}\label{sec:mechanisms}
%%% ====================================================================
...
\begin{proposition}[Local Euler factors]\label{prop:local-factors}
The $\zeta_{\mathbb{Q}(\sqrt{5})}(s)$ has the Euler product
...
\begin{proof}
The classical theory of $\zeta_{\mathbb{Q}(\sqrt{5})}(s)$ for quadratic fields; see
e.g.~\cite{Neukirch}.  The claim that $f_p$ is determined by $\psi_p$ follows
from the fact that $\chi_5(p) \equiv F_p \pmod{p}$
(\cref{def:frobenius}): whether $\golden^p$ is
...
\begin{equation}\label{eq:factorisation}
  \zeta(s) = \frac{\zeta_{\mathbb{Q}(\sqrt{5})}(s)}{L(s,\chi\_5)},
\end{equation}
...
component of a natural isomorphism $\eta_{\mathbb{F}_1}$ across all quadratic
...
With class number $h_{\mathbb{Q}(\sqrt{5})} = 1$, regulator $R_{\mathbb{Q}(\sqrt{5})} = \log\golden$, number of roots of unity
$w=2$, and discriminant $\Delta=5$:
\[
  \lim_{s\to 1}(s-1)\,\zeta_{\mathbb{Q}(\sqrt{5})}(s)
  = \frac{2^{r\_1}(2\pi)^{r\_2}hR}{w\sqrt{\Delta}}
...
\begin{equation}\label{eq:Lchi-even}
  L(2k,\chi_5)
  = \frac{(-1)^{k+1}\sqrt{5}\,(2\pi)^{2k}\,B_{2k,\chi\_5}}
         {2\,(2k)!\,5^{2k}},
\end{equation}
where $B_{2k,\chi\_5} \in \mathbb{Q}$ are the generalised Bernoulli numbers:
\[
  B_{n,\chi\_5} = 5^{n-1}\sum_{a=1}^{5}\chi_5(a)\,B_n\!\left(\tfrac{a}{5}\right).
\]
Computed values: $B_{2,\chi\_5}=\tfrac{4}{5}$,
$B_{4,\chi\_5}=-8$, $B_{6,\chi\_5}=\tfrac{804}{5}$, $B_{8,\chi\_5}=-5776$.
\end{theorem}
...
The doubled cosines of the regular pentagon evaluate to powers of~$\golden$:
...
\begin{equation}\label{eq:gauss-lerch}
  L(1,\chi_5)
  = -\frac{\tau(\chi\_5)}{f}\sum_{a=1}^{f-1}\chi_5(a)
...
% ------------------------------------------------------------------
\subsubsection{Numerical case study: from Ap\'ery's constant to \texorpdfstring{$\zeta(13)$}{zeta(13)}}\label{ssec:zeta3-example}
% ------------------------------------------------------------------

We trace the full chain from the pentagon to
Ap\'ery's constant $\zeta(3) = 1.2020569\ldots$ for the first few primes.
...
Their ratio gives
\[
  \zeta(3) = \frac{\zeta_{\mathbb{Q}(\sqrt{5})}(3)}{L(3,\chi\_5)} = 1.2020569\ldots
\]
...
The defining relations $i^2 = -1$ and $\golden^2 = \golden + 1$
...
$\{e_1,e_2,e\_3\}$. The coupling $z = \golden y$ identifies the
