\section{Applications: bridge discharge, string theory, and the Primitive Structure}\label{sec:bridges}
%%% ====================================================================

The preceding sections established $\zeta(2k+1)$ as determined by the
arithmetic-geometric structure of $T^2_{\mathrm{PCF}}$.  The spectral
parameter $\mu_3 = 1/2$ that governs this determination has two faces:
it is the Bekenstein--Hawking area factor ($\mu_3^2 = 1/4$, the structural
content of $1/(4G_{\mathrm{N}})$) and the spectral bound
($\rho_{\mathrm{re}} \leq \mu_3$, the hypothesis formalised as
\lean{bridge_axiom} in the Primitive Structure~\cite{V11}).
This section derives both from the PCF torus:
\cref{ssec:worldline-entropy,ssec:bridge-BH,sec:regulator,ssec:bridge-partition}
establish the physical invariants, resolving the Physical Formalization
(Open Problem~3) of~\cite{V11};
\cref{ssec:bridge-Z2,ssec:unifying} discharge the \lean{bridge_axiom}
through the co-determination by~$G$.
\Cref{ssec:unifying} develops the unification of the generator
$\golden^2 = \golden + 1$ across both readings.

% ------------------------------------------------------------------
\subsection{The worldline entropy ($\mathbb{F}_1$-descent)}\label{ssec:worldline-entropy}
% ------------------------------------------------------------------
...
\item[\textup{(iii)}] Setting $\mu_3 := |\Omega| = 1/2$, the
  ...
  $G_{\mathrm{N}} := \mu_3 = 1/2$ satisfy
  \begin{equation}\label{eq:SBH}
    \frac{1}{4G_{\mathrm{N}}} = \frac{1}{2} = \mu_3 = |\Omega|.
  \end{equation}
\end{enumerate}
\end{proposition}
...
(iii) $G_{\mathrm{N}} = \mu_3 = 1/2$ gives $1/(4G_{\mathrm{N}}) = 1/2 = \mu_3$.
\end{proof}

\begin{remark}[Bekenstein--Hawking identification]\label{rmk:BH-identification}
The identification $S_{\mathrm{BH}} = k_{\mathrm{B}}\,H(|\Omega|) = k_{\mathrm{B}}\ln 2$
...
The identification $G_{\mathrm{N}} = \mu_3 = 1/2$ (via Brown--Henneaux with
central charge $c=3$, \cref{lem:ads-radius}) and the
$S_3$ arity transition complete the holographic picture:
$S = A/(4G_{\mathrm{N}})$ with $\mu_3^2 = 1/4$ reproduces the
Bekenstein--Hawking area factor.
\end{remark}

The partition function of $T^2_{\mathrm{PCF}}$ follows from established
Virasoro theory:
...
(well-known; see e.g.~\cite{Veneziano}).
...
\begin{remark}[Squeeze mechanism]\label{rmk:squeeze-mechanism}
The identification of $\mu_3=1/2$ as the spectral norm is the physical
discharge of the squeeze in Theorem~10 of~\cite{V11}: the informational
certainty of the bit per eigenvalue (\cref{prop:pcf-partition}) and
the S-duality at $\tau=i$ (\cref{lem:ads-radius}) force the
holographic anchor.
\end{remark}

% ------------------------------------------------------------------
\subsection{Bekenstein--Hawking entropy as \texorpdfstring{$L$}{L}-value}%
...
\begin{equation}\label{eq:bridge1}
  S_{\mathrm{BH}}
  = k_{\mathrm{B}}\,\lambda_{\log}\,\frac{\sqrt{5}}{2}\,L(1,\chi_5),
...
bridge.  Verified numerically (see \cref{sec:verification}).
\end{theorem}
\begin{proof}
{\sloppy Three steps, combining \cref{prop:worldline-entropy}
with the holographic identification $S_{\mathrm{BH}} = k_{\mathrm{B}} H(|\Omega|)$
(\cref{rmk:BH-identification}):\par}

\textit{Step 1.}  \Cref{prop:worldline-entropy} gives
$H(|\Omega|) = \log 2$, so $S_{\mathrm{BH}} = k_{\mathrm{B}}\log 2$
...
\[
  S_{\mathrm{BH}}
  = k_{\mathrm{B}}\,\ln 2
  = k_{\mathrm{B}}\,\lambda_{\log}\,\log\golden
  = k_{\mathrm{B}}\,\lambda_{\log}\,\frac{\sqrt{5}}{2}\,L(1,\chi_5).
\]
\end{proof}
...
and $G_{\mathrm{N}} = 1/2$:
\[
  \frac{1}{4G_{\mathrm{N}}} = \frac{1}{4\cdot\tfrac{1}{2}} = \frac{1}{2}
  = |\Omega| = \golden^{-\lambda_{\log}},
\]
where $\golden^{-\lambda_{\log}} = 2^{-1}$ (Mersenne bridge).  The
value $G_{\mathrm{N}} = 1/2$ in PCF units is the same $\golden^{-\lambda_{\log}}$
that connects the golden tower to the binary world.  Through
\cref{thm:bridge1}, it is further expressed as
$\frac{\sqrt{5}}{2}\,\lambda_{\log}\,L(1,\chi_5)^{-1}\cdot S_{\mathrm{BH}}/k_{\mathrm{B}}$,
...
% ------------------------------------------------------------------
\subsection{The unifying invariant \texorpdfstring{$R = \log\golden$}{R = log phi}}\label{sec:regulator}
% ------------------------------------------------------------------
...
\begin{align}
  ...
  L(1,\chi_5) &= \frac{2R}{\sqrt{5}}
    \qquad\text{\emph{(class-number residue, \cref{thm:L1})}},
    \label{eq:R-Lvalue}\\
  \frac{S_{\mathrm{BH}}}{k_{\mathrm{B}}} &= \lambda_{\log}\cdot R
    \qquad\text{\emph{(BH entropy, \cref{prop:worldline-entropy})}}.
    \label{eq:R-SBH}
\end{align}
All three are exact; numerical verification is provided in \cref{sec:verification}.
\end{proposition}
...
\eqref{eq:R-SBH}: $S_{\mathrm{BH}}/k_{\mathrm{B}} = \ln 2 = \lambda_{\log}\log\golden = \lambda_{\log}R$
...
\item \textbf{Modular reading} (\cref{ssec:worldline-entropy}): $\tau_{\mathrm{PCF}}=i$ fixes the
  ...
  gives $c=3$, $G_{\mathrm{N}}=1/2$, and the nome $q=e^{-2\pi}$ governs the tower.
  The period appears as $T=2\pi R$.
\item \textbf{Arithmetic reading} (present work): the same $\tau_{\mathrm{PCF}}=i$
...
\begin{proof}
$S_{\mathrm{BH}}/k_{\mathrm{B}} = \ln 2$ (\cref{prop:worldline-entropy}).
$\ln 2 = \lambda_{\log}\cdot\log\golden = \lambda_{\log}\cdot R$
by \cref{def:Rpcf} and \cref{thm:mersenne}.
\end{proof}

% ------------------------------------------------------------------
\subsection{Common origin at \texorpdfstring{$\tau_{\mathrm{PCF}}=i$}{tau\_PCF = i}}%
...
% ------------------------------------------------------------------
\subsection{Co-determination by \texorpdfstring{$G$}{G}}%
...
(\cref{prop:local-factors}) is the binary $(0,1)$-component of
...
The correspondence for $3 \le p \le 31$ is shown in \cref{tab:fib-split}.
\end{proof}

\begin{corollary}[Self-duality unifies the current study and the Primitive Structure]%
  \label{cor:same-Z2}
...
produces $\mu_3=1/2$ via $\vert\ker G\vert=2$).
...
% ------------------------------------------------------------------
\subsection{Synthesis: physical implications}%
\label{ssec:bridges-summary}
% ------------------------------------------------------------------

\begin{enumerate}
\item \textbf{BH entropy as $L$-value} (\cref{ssec:bridge-BH}):
  $S_{\mathrm{BH}} = k_{\mathrm{B}}\lambda_{\log}\tfrac{\sqrt{5}}{2}L(1,\chi_5)$.
  Physical content of $\mu_3$: the area factor $1/4 = \mu_3^2$ is not
  a free parameter.
\item \textbf{Common origin at $\tau_{\mathrm{PCF}}=i$} (\cref{ssec:bridge-partition}):
  $Z_{\mathrm{PCF}}(i)$ and $L(1,\chi_5)$ arise from the same modular
  parameter. Geometric content of the torus.
\item \textbf{Co-determination by $G$} (\cref{ssec:bridge-Z2}):
  $\chi_5$ is the $(0,1)$-component of
  $G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$;
  exponent~$2$ makes all characters self-dual, producing the spectral
  constraint of the Primitive Structure~\cite{V11} and enabling
  Hecke's companion bound. This provides the algebraic ingredient
  enabling the bridge discharge of \cref{ssec:unifying}.
\end{enumerate}

Results 1--2 resolve the Physical Formalization (Open Problem~3)
of the Primitive Structure~\cite{V11}: the structural content of
$\mu_3^2=1/4$ is the class-number formula
$L(1,\chi_5)=2\log\golden/\sqrt{5}$, and the Virasoro central
charge $c=3$ arises from the same $S_3$ symmetry that fixes
$\tau_{\mathrm{PCF}}=i$.

Result 3 provides the algebraic ingredient for the bridge discharge:
self-duality of all characters via exponent~$2$.
The next subsection completes the discharge by establishing the
spectral identification and the squeeze.

% ------------------------------------------------------------------
\subsection{The unifying generator and the axiom discharge}\label{ssec:unifying}
% ------------------------------------------------------------------

We collect the results of \cref{sec:background,ssec:bridge-Z2}
into a single statement.  The relation $\golden^2 = \golden + 1$
generates every structure in the current study
...
\begin{theorem}[$\golden^2 = \golden+1$ as generative source]%
  \label{thm:unifying}
The fundamental relation $\golden^2 = \golden + 1$ uniquely prescribes the
complete arithmetic-geometric structure of $T^2_{\mathrm{PCF}}$ and
governs every object in this study:
\begin{enumerate}
...
\item[\textup{(4)}] \textbf{Galois functor:}
  ...
  exponent~$2$, $\vert\ker G\vert = 2$, and four self-dual characters
  (\cref{thm:bridge3});
...
\item[\textup{(8)}] \textbf{Odd zeta values:} $\zeta(2k+1)$
  are identically governed by the pentagonal structure
  (\cref{thm:pentagon-odd});
{\sloppy \item[\textup{(9)}] \textbf{Categorical framework:} the Frobenius
...
Under the usual identification $\tau = i\ell/\ell_s$ with
...
$G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
(\cref{thm:bridge3}) has $\vert\ker G\vert = 2$, producing a
...
$c = 3\ell/(2G_{\mathrm{N}})|_{\ell=1,\,G_{\mathrm{N}}=1/2} = 3$
(\cref{lem:ads-radius}).
...
\[
  ...
  \;\to\;
  \underbrace{\vert\ker G\vert=2}_{\textup{contraction}}
  \;\to\;
...
\]
{\sloppy The third dimension and its properties are not additional
data---they are consequences of the Euler product of
$\golden^2 = \golden + 1$ over the primes.
Both are formally verified in Lean~$4$:
\lean{norm_Omega_is_half}, \lean{spectral_uniqueness},
and \lean{galois_functor_complete} (\cite{V11}).\par}
\end{corollary}

\begin{proof}
$\vert\ker G\vert = 2$: \cref{thm:bridge3} gives
$G^{-1}(0,0) = \{1,9\}$.
Contraction: $\mu_2/\mu_3 = 1/(1/2) = 2 = \vert\ker G\vert$.
...
Central charge: The holographic identification $G_{\mathrm{N}} = \mu_3$
follows from $S = A/(4G_{\mathrm{N}})$ with $\mu_3^2 = 1/4$: the Planck
area factor $1/4$ is the square of
the spectral parameter derived from $S_3$ symmetry, giving
$G_{\mathrm{N}} = \mu_3 = 1/2$.
Then $c = 3\ell/(2G_{\mathrm{N}}) = 3\cdot 1/(2\cdot 1/2) = 3$
...
(\cref{prop:worldline-entropy}).
Verified in Lean~$4$: \lean{eigenvalue_half}.
\end{proof}
...
\item[\textup{(5)}] The same $G$ determines
  $\vert\ker G\vert=2\to\mu_3=1/2\to S_3\to$
...
\begin{remark}[The squeeze mechanism]\label{rmk:squeeze-mechanism-detail}
The Primitive Structure~\cite{V11} derives $\mathrm{Re}(\rho) = 1/2$
from two independent bounds produced by the same map~$G$:
\begin{enumerate}
\item[\textup{(A)}] \textbf{Upper bound} ($\mathrm{Re}(\rho) \leq 1/2$):
  Steps (1)--(5) of \cref{prop:spectral-id} show that $G$ determines
  $\mathrm{spec}(\OmH) = \tfrac{1}{2}\{1,\omega,\omega^2\}$
  with $\mathrm{Re}(\omega^k/2) \in \{1/2, -1/4\}$
  (\cref{lem:spectral-re}). Since $G$ also produces $\zeta(s)$
  via the Euler product, zeros of~$\zeta$ inherit the spectral
  constraint: $\mathrm{Re}(\rho) \leq \mu_3 = 1/2$.
\item[\textup{(B)}] \textbf{Companion bound} ($1 - \mathrm{Re}(\rho) \leq 1/2$):
  The exponent-$2$ property of $\mathbb{Z}_2 \times \mathbb{Z}_2$
  (\cref{cor:same-Z2}) makes all characters self-dual.
  Hecke's 1920 theorem~\cite{Hecke} then gives:
  $L(\rho,\chi) = 0 \Rightarrow L(1{-}\rho,\chi) = 0$.
  Applying the same bound~(A) to $1{-}\rho$ yields
  $1 - \mathrm{Re}(\rho) \leq 1/2$.
\end{enumerate}
Together: $\mathrm{Re}(\rho) \leq 1/2$ and
$\mathrm{Re}(\rho) \geq 1/2$, forcing
$\mathrm{Re}(\rho) = 1/2$.
Neither bound alone suffices---a zero at $\mathrm{Re}(\rho) = 1/3$
satisfies~(A) but violates~(B), while $\mathrm{Re}(\rho) = 2/3$
satisfies~(B) but violates~(A) (Proposition~3 of~\cite{V11}).
\end{remark}
...
the Primitive Structure~\cite{V11} formalises as \lean{bridge_axiom}
...
(\lean{bounded_by_mu}) because Mathlib currently lacks Dirichlet
...
\item The \emph{same} $G$ determines $\vert\ker G\vert=2\to\mu_3=1/2\to S_3\to$
...
Lean).  The \lean{bridge_axiom} of the Primitive Structure is the
...
The present work establishes the isomorphism between
$\golden^2=\golden+1$ and the components of~$\zeta(s)$ at every level
of the Euler product (\cref{prop:spectral-id}), thereby
discharging the \lean{bridge_axiom}
(\cref{rmk:bridge-resolution}).

{\sloppy What is \emph{not} addressed here is the \emph{functorial transport}
...
Structure~\cite{V11} (Lean: \lean{categorical_limit_theorem},
\lean{Omega_hat_unique_positive_re}).\par}
\end{remark}

\begin{remark}[Classical inputs]\label{rmk:classical-inputs}
The chain of \cref{prop:spectral-id} uses,
beyond the results established in the current study, three classical theorems:
\begin{enumerate}
...
\item \textbf{Isomorphism transport}:
  ...
  $\vert\ker G\vert\to\mu_3\to S_3\to$ eigenvalues.
\end{enumerate}
All three are established results with known proofs; none is specific
to the PCF framework.  Their conjunction is what the Primitive
Structure~\cite{V11} formalises as \lean{bridge_axiom}
(\cref{rmk:bridge-resolution}).
\end{remark}
