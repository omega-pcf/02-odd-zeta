...
discharging the \texttt{bridge\_axiom} and resolving the Physical Formalization
...
% ------------------------------------------------------------------
\subsection{Geometric unity at \texorpdfstring{$\tau_{\mathrm{PCF}}=i$}{tau_PCF = i}}%
...
Together, these physical results discharge the \texttt{bridge\_axiom} and resolve the
...
discharges the \texttt{bridge\_axiom} of the Primitive Structure~\cite{V11} and
...
and \texttt{galois\_functor\_complete} (\cite{V11}).\par}
\end{corollary}
...
(\cref{prop:worldline-entropy}).
Verified in Lean~$4$: \texttt{eigenvalue\_half}.
\end{proof}
...
\texttt{Omega\_hat\_unique\_positive\_re}).\par}
\end{remark}
