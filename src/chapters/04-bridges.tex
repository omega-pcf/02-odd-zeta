...
discharging the \lean{bridge\_axiom} and resolving the Physical Formalization
...
\begin{align}
  ...
  L(1,\chi_5) &= \frac{2R}{\sqrt{5}}
    ...
    \label{eq:R-Lvalue}\\
  \frac{S_{\mathrm{BH}}}{k\_B} &= \lambda_{\log}\cdot R
...
function $Z(\tau) = \mathrm{Tr}\,q^{L\_0-c/24}\bar q^{\bar L\_0-c/24}$
...
four characters $\{1,\chi_4,\chi_5,\chi_4\chi\_5\}$
...
Together, these physical results discharge the \lean{bridge\_axiom} and resolve the
...
discharges the \lean{bridge\_axiom} of the Primitive Structure~\cite{V11} and
...
\lean{norm\_Omega\_is\_half}, \lean{spectral\_uniqueness},
and \lean{galois\_functor\_complete} (\cite{V11}).\par}
\end{corollary}
...
(\cref{prop:worldline-entropy}).
Verified in Lean~$4$: \lean{eigenvalue\_half}.
\end{proof}
...
the Primitive Structure~\cite{V11} formalises as \lean{bridge\_axiom}
...
(\lean{bounded\_by\_mu}) because Mathlib currently lacks Dirichlet
...
Lean).  The \lean{bridge\_axiom} of the Primitive Structure is the
...
discharging the \lean{bridge\_axiom}
...
Structure~\cite{V11} (Lean: \lean{categorical\_limit\_theorem},
\lean{Omega\_hat\_unique\_positive\_re}).\par}
\end{remark}
...
Structure~\cite{V11} formalises as \lean{bridge\_axiom}
(\cref{rmk:bridge-resolution}).
\end{remark}
