...
discharging the \lean{bridge_axiom} and resolving the Physical Formalization
(Open Problem~3) of the Primitive Structure~\cite{V11}: making rigorous the
...
% ------------------------------------------------------------------
\subsection{Bekenstein--Hawking entropy and \texorpdfstring{$L$}{L}-values}%
...
% ------------------------------------------------------------------
\subsection{Geometric unity at \texorpdfstring{$\tau_{\mathrm{PCF}}=i$}{tau\_PCF = i}}%
...
% ------------------------------------------------------------------
\subsection{Algebraic symmetry via \texorpdfstring{$\mathbb{Z}_2\times\mathbb{Z}_2$}{Z2 x Z2}}%
...
\begin{enumerate}
\item \textbf{Intrinsic correspondence:}
  $S_{\mathrm{BH}} = k_{\mathrm{B}}\lambda_{\log}\tfrac{\sqrt{5}}{2}L(1,\chi_5)$.
  Derived from $|\Omega|=1/2$ (\cref{prop:worldline-entropy})
  and the Mersenne bridge; with vanishing error (see \cref{sec:verification}).
\item \textbf{Geometric unity:}
...
\item \textbf{Algebraic symmetry via $\mathbb{Z}_2\times\mathbb{Z}_2$:}
  $\chi_5$ is the binary $(0,1)$-component of $G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
  (\cref{thm:bridge3});
  exponent~$2$ makes $\chi_5$ self-dual here and produces the spectral
  constraint of the Primitive Structure~\cite{V11}.
\end{enumerate}

Together, these physical results discharge the \lean{bridge_axiom} and resolve the
Physical Formalization (Open Problem~3) of the Primitive Structure~\cite{V11}:
the structural content of $\mu_3^2=1/4$ (equivalently $1/(4G_{\mathrm{N}})=\mu_3=1/2$) is the class-number
formula $L(1,\chi_5)=2\log\golden/\sqrt{5}$ expressed as
$S_{\mathrm{BH}}/k_{\mathrm{B}} = \lambda_{\log}(\sqrt5/2)L(1,\chi_5)$
(Intrinsic correspondence); the Virasoro central charge $c=3$ arises from the same
$S_3$ symmetry of $T^2_{\mathrm{PCF}}$ that fixes $\tau_{\mathrm{PCF}}=i$
and hence both $Z_{\mathrm{PCF}}(i)$ and $L(1,\chi_5)$ (Geometric unity);
and the $\chi_5$-character organising odd zeta values is the
binary $(0,1)$-component of the Galois functor whose exponent-$2$ property
discharges the \lean{bridge_axiom} of the Primitive Structure~\cite{V11} and
produces the self-duality of $L(1,\chi_5)$ (Algebraic symmetry).
