...
(Definition~\ref{def:torus}) satisfies:
\begin{enumerate}
...
follows from Proposition~\ref{prop:worldline-entropy}: the binary
...
central charge $c=3$, Lemma~\ref{lem:ads-radius}) and the
...
At $c = D = 3$ (Corollary~\ref{cor:S3-from-euler}) and
$\tau = i$ (Lemma~\ref{lem:ads-radius}), the vacuum energy
...
\begin{proof}
Three steps, combining Proposition~\ref{prop:worldline-entropy}
with the holographic identification $S_{\mathrm{BH}} = k_B H(|\Omega|)$
(Remark~\ref{rmk:BH-identification}):

\textit{Step 1.}  Proposition~\ref{prop:worldline-entropy} gives
$H(|\Omega|) = \log 2$, so $S_{\mathrm{BH}} = k_B\log 2$
(Remark~\ref{rmk:BH-identification}).

\textit{Step 2.}  The Mersenne bridge (Theorem~\ref{thm:mersenne}) gives
$\golden^{\lambda_{\log}} = 2$, hence
$\ln 2 = \lambda_{\log}\cdot\log\golden$.

\textit{Step 3.}  Theorem~\ref{thm:L1} gives
...
\[
  ...
  = k_B\,\lambda_{\log}\,\frac{\sqrt{5}}{2}\,L(1,\chi_5).\qquad\square
\]
\end{proof}
...
formula is not an independent constant.  Via~\eqref{eq:bridge1}
and $G_N = 1/2$:
\[
...
Theorem~\ref{thm:bridge1}, it is further expressed as
...
The three bridges of Section~\ref{sec:bridges} are consequences of a single
...
\begin{proof}
\eqref{eq:R-torus}: Definition~\ref{def:torus}: $T_{\mathrm{PCF}}=2\pi\log\golden=2\pi R$.
\eqref{eq:R-Lvalue}: Theorem~\ref{thm:L1}: $L(1,\chi_5)=2\log\golden/\sqrt{5}=2R/\sqrt{5}$.
\eqref{eq:R-SBH}: $S_{\mathrm{BH}}/k_B = \ln 2 = \lambda_{\log}\log\golden = \lambda_{\log}R$
by Proposition~\ref{prop:worldline-entropy} and Theorem~\ref{thm:mersenne}.
\qed
\end{proof}
...
\begin{enumerate}
\item \textbf{Modular reading} (§\ref{ssec:worldline-entropy}): $\tau_{\mathrm{PCF}}=i$ fixes the
...
\item \textbf{Arithmetic reading} (this paper): the same $\tau_{\mathrm{PCF}}=i$
  ...
  of $\ZKsqrt(s)$.  The period appears as $R = T/(2\pi)$.
\end{enumerate}
...
\begin{proof}
$S_{\mathrm{BH}}/k_B = \ln 2$ (Proposition~\ref{prop:worldline-entropy}).
$\ln 2 = \lambda_{\log}\cdot\log\golden = \lambda_{\log}\cdot R$
by Definition~\ref{def:Rpcf} and Theorem~\ref{thm:mersenne}.
\qed
\end{proof}
...
(Theorem~\ref{prop:pcf-partition}) and the $L$-value $L(1,\chi_5)$
(Theorem~\ref{thm:L1}) arise from the same modular parameter
...
$\tau\mapsto -1/\tau$ (Lemma~\ref{lem:ads-radius}).  Evaluating the Virasoro partition
function $Z(\tau) = \mathrm{Tr}\,q^{L_0-c/24}\bar q^{\bar L_0-c/24}$
at $\tau=i$ with $c=D=3$ gives $Z_{\mathrm{PCF}}(i)=e^{-3\pi/2}|\eta(i)|^{-6}$
(Theorem~\ref{prop:pcf-partition}).  The modular parameter $\tau=i$ is
...
$T = 2\pi\log\golden$ (Definition~\ref{def:torus}).  The class-number
formula for $\mathbb{Q}(\sqrt{5})$ then gives
$L(1,\chi_5) = T/(\pi\sqrt{5})$ (Theorem~\ref{thm:L1}).
...
(Proposition~\ref{prop:local-factors}) is the component $(0,1)$ of
...
Verified for all primes $3\le p\le 31$ in Table~\ref{tab:fib-split}.
\qed
\end{proof}
...
Definition~\ref{def:pcf-span}: $G$ is the common source of
...
\item \textbf{Bridge~1 (exact, self-contained):}
  ...
  Derived from $|\Omega|=1/2$ (Proposition~\ref{prop:worldline-entropy})
...
We collect the results of §§\ref{sec:background}--\ref{ssec:bridge-Z2}
into a single statement.  The relation $\golden^2 = \golden + 1$
generates every structure in this paper
(Theorem~\ref{thm:unifying} below), and the dimensional chain from
1D through 2D to 3D follows as a consequence of the Euler product
over the primes (Corollary~\ref{cor:S3-from-euler}).
...
\item[\textup{(4)}] \textbf{Galois functor:}
  ...
  \textup{(Theorem~\ref{thm:bridge3})};
\item[\textup{(5)}] \textbf{Torus:}
  $\Omega(\tau) = \tfrac{1}{2}e^{i\tau\log\golden}$ closes
  with period $T = 2\pi\log\golden$
  \textup{(Definition~\ref{def:torus})};
\item[\textup{(6)}] \textbf{Norm:} $|\Omega| = 1/2$
  \textup{(Proposition~\ref{prop:worldline-entropy})};
\item[\textup{(7)}] \textbf{Euler product:}
  $\ZKsqrt(s) = \prod_p f_p(s)$ and
  $\zeta(s) = \ZKsqrt(s)/L(s,\chi_5)$
  \textup{(Theorem~\ref{thm:factorisation})};
\item[\textup{(8)}] \textbf{Odd zeta values:} $\zeta(2k+1)$
  structurally determined
  \textup{(Theorem~\ref{thm:pentagon-odd})};
\item[\textup{(9)}] \textbf{Categorical framework:} the Frobenius
  ...
  Section~\ref{sec:categorical-framework}.
\end{enumerate}
...
Section~\ref{sec:categorical-framework}: the functoriality of $\Psi$
...
(Definition~\ref{def:torus}) is the unique fixed point of
...
The Euler product $\ZKsqrt(s) = \prod_p f_p(s)$ is the collective
...
(Theorem~\ref{thm:bridge3}) has $|\ker G| = 2$, producing a
...
(Proposition~\ref{prop:dim-chain}),
...
\begin{proof}
$|\ker G| = 2$: Theorem~\ref{thm:bridge3} gives
$G^{-1}(0,0) = \{1,9\}$.
...
(Brown--Henneaux with $\ell = 1$ (Lemma~\ref{lem:ads-radius})).
\qed
\end{proof}
...
(Proposition~\ref{prop:worldline-entropy}).
...
\item[\textup{(1)}] $G$ classifies every prime $p > 5$ by
  ...
  \textup{(Theorem~\ref{thm:bridge3},
  Proposition~\ref{prop:fib-split})}.
\item[\textup{(2)}] The splitting type determines the local
  Euler factor $f_p(s)$
  \textup{(Proposition~\ref{prop:local-factors})}.
\item[\textup{(3)}] The Euler product
  $\ZKsqrt(s) = \prod_p f_p(s)$ and the factorisation
  $\zeta(s) = \ZKsqrt(s)/L(s,\chi_5)$ hold for
  $\mathrm{Re}(s)>1$ \textup{(Theorem~\ref{thm:factorisation};
  Euler~\cite{Euler})}.
\item[\textup{(4)}] Analytic continuation determines $\zeta(s)$
  on all of $\mathbb{C}$ from its values on $\mathrm{Re}(s)>1$
  \textup{(identity theorem; Riemann~\cite{Riemann};
  Remark~\ref{rmk:euler-determines})}.
\item[\textup{(5)}] The same $G$ determines
  $|\ker G|=2\to\mu_3=1/2\to S_3\to$
  eigenvalues $\omega^k/2$
  \textup{(Corollary~\ref{cor:S3-from-euler})}; the unique
  eigenvalue with $\mathrm{Re}>0$ has $\mathrm{Re}=1/2$
  \textup{(Lemma~\ref{lem:spectral-re})}.
\end{enumerate}
...
Theorem~\ref{thm:factorisation} respectively.
Step (4) is the identity theorem for Dirichlet series
(Remark~\ref{rmk:euler-determines}).
Step (5) is Corollary~\ref{cor:S3-from-euler} and
Lemma~\ref{lem:spectral-re}.
\qed
\end{proof}

\begin{remark}[Discharge of the bridge axiom]\label{rmk:bridge-resolution}
Proposition~\ref{prop:spectral-id} makes explicit the mechanism that
...
(Proposition~15) states that zeros of $L$-functions factorising
...
\item $G$ classifies every prime $p>5$ by splitting type in
  ...
  (Proposition~\ref{prop:local-factors}).
...
\item The \emph{same} $G$ determines $|\ker G|=2\to\mu_3=1/2\to S_3\to$
  eigenvalues $\omega^k/2$ (Corollary~\ref{cor:S3-from-euler}).
\end{enumerate}
...
of the Euler product (Proposition~\ref{prop:spectral-id}), thereby
discharging the \texttt{bridge\_axiom}
(Remark~\ref{rmk:bridge-resolution}).
...
The chain of Proposition~\ref{prop:spectral-id} uses,
...
\item \textbf{Isomorphism transport}:
  ...
  (Proposition~\ref{prop:spectral-id}): primes, splitting types,
