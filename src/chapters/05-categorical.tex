\section{The Categorical Framework}\label{sec:categorical-framework}
%%% ====================================================================

The preceding sections established that $G\colon\Ztwenty\to\mathbb{Z}_2\times\mathbb{Z}_2$
determines both the Euler product of $\ZKsqrt(s)$ (upper branch of the
diagram in~\S\ref{sec:objective}) and the spectral structure
$\mu_3=1/2$, $S_3$, $\omega^k/2$ (lower branch).
We now make the categorical content of this determination explicit:
the Frobenius system is a functor~(\S\ref{ssec:frobenius-functor}),
the Euler product is a colimit~(\S\ref{ssec:euler-colimit}),
the Dedekind factorisation is a natural
isomorphism~(\S\ref{ssec:dedekind-natural}),
and the two branches form a span with common
source~(\S\ref{ssec:span}).
...
By spectral uniqueness (Corollary~\ref{cor:S3-from-euler}), the only
...
$\norm{\Omega}=1/2<1$ (Proposition~\ref{prop:worldline-entropy}).
...
The partial Euler functor $E$ (Definition~\ref{def:partial-euler})
...
(Proposition~\ref{prop:fib-split}) and $f_p$ is determined by
$\chi_5(p)$ (Proposition~\ref{prop:local-factors}).
\end{lemma}
...
\begin{equation}\label{eq:euler-colimit}
  \ZKsqrt(s) = \varinjlim_{S\in\mathcal{P}} E(S)(s).
\end{equation}
...
The colimit $\ZKsqrt(s) = \varinjlim E(S)$ in $\mathbf{DirSer}$
parallels the tower limit $\norm{\Omega}_k \to 0$ in the spectral
category of the Primitive Structure~\cite{V11}
(\texttt{categorical\_limit\_theorem}).
...
$\mathbb{Q}(\sqrt{5})$ & $\chi_5$ &
  $\ZKsqrt = \zeta\cdot L(s,\chi_5)$ (\textbf{this paper}) \\
...
(Theorem~\ref{thm:bridge3}), one for each fiber of~$G$.
\end{theorem}
...
The identity $\zeta(s) = \ZKsqrt(s)/L(s,\chi_5)$
(Theorem~\ref{thm:factorisation}) is the
...
\item[\textup{(i)}] $F_{\mathrm{arith}}$:
  ...
  \{f_p\} \xrightarrow{\varinjlim} \ZKsqrt(s)
...
\item[\textup{(a)}] $F_{\mathrm{arith}}$ depends on $\chi_5=\pi_2\circ G$
  (Theorem~\ref{thm:bridge3}, Propositions~\ref{prop:local-factors}
  and~\ref{prop:euler-colimit}).
\item[\textup{(b)}] $F_{\mathrm{spec}}$ depends on
  $|\ker G|=2$ (Corollary~\ref{cor:S3-from-euler},
...
\begin{proof}
(a) Theorem~\ref{thm:bridge3} $\to$
Proposition~\ref{prop:fib-split} $\to$
Proposition~\ref{prop:local-factors} $\to$
Proposition~\ref{prop:euler-colimit}.
(b) $|\ker G|=2$ $\to$ $\mu_3=1/2$ $\to$ arity uniqueness $\to$
$S_3$ $\to$ $\omega^k/2$.
(c) Lean identifiers from
\texttt{PCF\_Complete\_v11\_Unified.lean}~(\cite{V11}).
\qed
\end{proof}
...
% ------------------------------------------------------------------
\subsection{What the categorical framework establishes}\label{ssec:cat-discussion}
% ------------------------------------------------------------------
...
Definition~7 of~\cite{V11} (\texttt{TernaryLZero}) is established
by the co-determination theorem (Theorem~\ref{thm:co-determination}):
...
Proposition~\ref{prop:euler-colimit} exhibits the Euler product as a
colimit of a diagram whose source is~$G$: each $f_p$ is determined by
$\chi_5(p) = \pi_2(G(p\bmod 20))$
(Proposition~\ref{prop:local-factors}), and primes in the same
...
by~$G$ (Proposition~\ref{prop:euler-colimit}), and the \emph{same}~$G$
...
(Definition~\ref{def:pcf-span}) plays the structural role of
...
\item[\textup{(i$'$)}] The span
  ...
  $|\ker G|=2$ (Theorem~\ref{thm:bridge3}), verified
...
\item[\textup{(ii$'$)}] The upper bound
  ...
  (Theorem~\ref{thm:co-determination}): because $G$ produces
  both $\zeta(s)$ (via the Euler product colimit,
  Proposition~\ref{prop:euler-colimit}) and
  $\mathrm{spec}(\OmH) = \tfrac{1}{2}\{1,\omega,\omega^2\}$
  (via $|\ker G|=2$, Corollary~\ref{cor:S3-from-euler}),
  ...
  (Lemma~\ref{lem:spectral-re}); the three classical
  theorems---Euler, Gelfand, Dunford--Schwartz---operate on
  this concrete structure (\S\ref{ssec:cat-discussion},
  Remark~\ref{rmk:classical-bridge}).
\item[\textup{(iii$'$)}] The lower bound
  $1-\mathrm{Re}(\rho)\le\mu_3$, from the exponent-$2$ property
  of $\mathbb{Z}_2\times\mathbb{Z}_2$
  (Corollary~\ref{cor:same-Z2}): all characters are self-dual,
  ...
  (Proposition~3 of~\cite{V11}; \S\ref{sec:verification}).
\end{enumerate}
...
$\mu_3 = 1/2$ (Lemma~\ref{lem:spectral-re}), are the
...
$C\times C$ (arithmetic surface)
  & Span with common source $G$ (Def.~\ref{def:pcf-span}) \\
Horizontal $C\times\{P\}$
  & $F_{\mathrm{arith}}\to\mathbf{DirSer}$
    (Prop.~\ref{prop:euler-colimit}) \\
Vertical $\{P\}\times C$
  & $F_{\mathrm{spec}}\to\mathrm{Spec}(\OmH)$
    (Cor.~\ref{cor:S3-from-euler}) \\
Diagonal $\Delta$
  & Common source $G$ (Thm.~\ref{thm:bridge3}) \\
$\mathrm{Fr}\colon x\mapsto x^q$
  & $G\colon\Ztwenty\to\mathbb{Z}_2{\times}\mathbb{Z}_2$
    (Def.~5 of~\cite{V11}) \\
$|{\alpha_i}|\le q^{1/2}$ (intersection)
  & $\mathrm{Re}(\rho)\le\mu_3$ (Thm.~\ref{thm:co-determination}) \\
Functional eq.\ $\to |{\alpha_i}|\ge q^{1/2}$
  & Hecke $\to 1{-}\mathrm{Re}(\rho)\le\mu_3$
    (Cor.~\ref{cor:same-Z2}) \\
$|{\alpha_i}|=q^{1/2}$
  & $\mathrm{Re}(\rho)=1/2$ (Thm.~10 of~\cite{V11}) \\
Eigenvalues on $H^1_{\text{\'et}}$
  & Eigenvalues of $\OmH$ (Lem.~\ref{lem:spectral-re}) \\
All $|{\alpha_i}|$ equal
...
geometry; the span of Definition~\ref{def:pcf-span} provides a
...
(Proposition~\ref{prop:Psi-functorial}),
...
Remark~\ref{rmk:standard-id}): a one-dimensional vector whose
...
(Theorem~\ref{thm:hurwitz}): $L = (\text{Hurwitz sum})/5^s$.
...
Since $\ZKsqrt(s) = \zeta(s)\cdot L(s,\chi_5)$
(Theorem~\ref{thm:factorisation}), the transcendence of
$L(2k+1,\chi_5)$ (proved at $s=1$; structurally persistent for
$s\ge 3$) implies that $\zeta(2k+1)$ and $\ZKsqrt(2k+1)$
cannot both be algebraic.  The structural determination
$\zeta(2k+1) = \ZKsqrt(2k+1)/L(2k+1,\chi_5)$ is not a
