\section{Formalism: the categorical framework}\label{sec:categorical-framework}
%%% ====================================================================
...
triples $(\sigma,\mu,\{\lambda\_k\})$ satisfying $\sigma+\mu=2$,
...
$\Psi(p_1^{a_1}\cdots p_k^{a\_k}) = \psi_{p_1}^{a_1}\circ\cdots\circ\psi_{p\_k}^{a\_k}$.
\end{proposition}
...
\begin{remark}[$\mathbb{F}_1$-descent data: Borger's criteria]\label{rmk:lambda-ring}
In the language of Borger~\cite{Borger}, $\Psi$ restricted to primes
equips $\Rpcf$ with a $\Lambda$-ring structure constituting
$\mathbb{F}_1$-descent data. The three defining criteria are
satisfied by the pentagonal arithmetic of $\golden$:
\begin{enumerate}
\item[\textup{(1)}] \emph{Purely multiplicative} (no addition):
  $\psi_p(\golden) = \golden^p$ is a power map.
  The fragment $\Ztwenty$ is closed under multiplication but
  destroyed under addition
  (\cref{prop:Z20-arithmetic};
  Lean: \lean{add_destroyed}, verified by \lean{decide}
  over all $64$ pairs).
\item[\textup{(2)}] \emph{Functoriality}: $\psi_p\circ\psi_q = \psi_{pq}$
  (\cref{prop:Psi-functorial};
  Lean: \lean{frobenius_composition}, \lean{pow_mul}).
\item[\textup{(3)}] \emph{Frobenius congruence}:
  $\psi_p(a)\equiv a^p\pmod{p}$, concretely
  $\chi_5(p)\equiv F_p\pmod{p}$
  (\cref{prop:fib-split};
  Lean: \lean{fib_split_criterion}, \lean{native_decide}).
\end{enumerate}
The collected statement is \lean{F1_descent_data} in
\lean{odd_zeta_pcf_v1.lean}: each criterion is discharged
by a verified Lean theorem.
...
$\mathbb{Q}(\sqrt{5})$ & $\chi_5$ &
  $\zeta_{\mathbb{Q}(\sqrt{5})} = \zeta\cdot L(s,\chi_5)$ (\textbf{present work}) \\
...
\item[\textup{(i)}] $F_{\mathrm{arith}}$:
  ...
  \{f\_p\} \xrightarrow{\varinjlim} \zeta_{\mathbb{Q}(\sqrt{5})}(s)
...
\item[\textup{(c)}] Both reduce to properties of $G$
  ...
  in Lean~$4$:
  \lean{G_hom}, \lean{G_surj},
  \lean{G_kernel} (\cite{V11}).
\end{enumerate}
\end{theorem}
...
\lean{bridge_axiom} of the Primitive Structure~\cite{V11}:
the hypothesis \lean{bounded_by_mu} ($\rho_{\mathrm{re}}\le\mu_3$)
is not an independent assumption but the conjunction of
(a)~the Euler product colimit (present work),
...
bound.  The \lean{bridge_axiom} thereby becomes a theorem.
...
justification for \lean{bridge_axiom}: Euler~\cite{Euler},
Gelfand~\cite{Gelfand}, and Dunford--Schwartz~\cite{DunfordSchwartz}.
The categorical framework developed in the current study identifies the
...
The three classical theorems are the tools; the current study provides the
...
\item[\textup{(i)}] The self-product $C\times C$ with
  ...
  and diagonal~$\Delta$;
\item[\textup{(ii)}] An upper bound $|{\alpha\_i}|\le q^{1/2}$
  from the positivity of the intersection form;
\item[\textup{(iii)}] A lower bound $|{\alpha\_i}|\ge q^{1/2}$
...
mirroring Weil's squeeze $|{\alpha\_i}| = q^{1/2}$.
...
$|{\alpha\_i}|\le q^{1/2}$ (intersection)
  & $\mathrm{Re}(\rho)\le\mu_3$ (\cref{thm:co-determination}) \\
Functional eq.\ $\rightarrow |{\alpha\_i}|\ge q^{1/2}$
...
% ------------------------------------------------------------------
\subsection{The polylogarithmic circuit: from \texorpdfstring{$\mathbb{F}_1$}{F1} to transcendence}%
