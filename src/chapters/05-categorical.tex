...
$\Psi(p_1^{a\_1}\cdots p_k^{a\_k}) = \psi_{p\_1}^{a\_1}\circ\cdots\circ\psi_{p\_k}^{a\_k}$.
\end{proposition}
...
(1) $\psi_p(\golden) = \golden^p$ (purely multiplicative),
(2) $\psi_p\circ\psi_q = \psi_{pq}$ (functorial), and
(3) $\psi_p(a)\equiv a^p\pmod{p}$ (Frobenius congruence),
constituting $\mathbb{F}_1$-descent data.
The decidability of this descent is ensured by the
\emph{additive destruction} of $\Ztwenty$ (\cref{prop:Z20-arithmetic}),
which isolates the multiplicative fragment
(Lean: \lean{F1_descent\_data}).
\end{remark}
...
\end{tabular}
\end{center}
The four characters $\{1,\chi_4,\chi_5,\chi_4\chi\_5\}$ are exactly
...
\item[\textup{(i)}] $F_{\mathrm{arith}}$:
  ...
  \mathbf{B}(\mathbb{Z}_2{\times}\mathbb{Z}_2) \xrightarrow{\chi\_5}
...
\item[\textup{(c)}] Both reduce to properties of $G$
  ...
  in Lean~$4$:
  \lean{G\_hom}, \lean{G\_surj},
  \lean{G\_kernel} (\cite{V11}).
\end{enumerate}
\end{theorem}
...
\lean{PCF_Complete_v11\_Unified.lean}~(\cite{V11}).
\end{proof}
...
\lean{bridge\_axiom} of the Primitive Structure~\cite{V11}:
the hypothesis \lean{bounded\_by\_mu} ($\rho_{\mathrm{re}}\le\mu_3$)
...
bound.  The \lean{bridge\_axiom} thereby becomes a theorem.
...
justification for \lean{bridge\_axiom}: Euler~\cite{Euler},
...
into exactly four characters $\{1,\chi_4,\chi_5,\chi_4\chi\_5\}$---the
