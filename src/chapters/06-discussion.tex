...
identified in \cref{op:polylog-baker}
and \cref{op:periods}.
...
The algebraic and spectral results of \cref{sec:bridges,ssec:unifying} have been verified in Lean~$4$ with
...
\texttt{odd\_zeta\_pcf\_v1.lean}.

\begin{table}[ht]
\centering
\caption{Lean audit: correspondence between paper results and
formalised identifiers.  All entries have $0$~\texttt{sorry}.
...
$|\Ztwenty|=8$ & \texttt{Z20star\_card} & \texttt{decide} \\
Mult.\ closed & \texttt{mul\_closed} & \texttt{decide} \\
...
Exponent~$2$ & \texttt{exponent\_two} & \texttt{decide} \\
...
$\forall p>5$: $p\bmod 20\in\Ztwenty$ & \texttt{primes\_land\_in\_Z20star} & \texttt{omega+decide} \\
...
\texttt{G\_kernel} in \texttt{odd\_zeta\_pcf\_v1.lean};
equivalently \texttt{G\_homomorphism}, \texttt{G\_surjective},
...
with $0$~\texttt{sorry} and one axiom ($\golden^2=\golden+1$);
