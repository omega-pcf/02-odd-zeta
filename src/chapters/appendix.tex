...
\midrule
\lean{odd_zeta_pcf_v1.lean} & 665 & 63 & 0 & Verified \\
...
$c = 3\ell/(2G_{\mathrm{N}}) = 3$ & \lean{brown_henneaux_c} & \lean{norm_num} \\
$1/(4G_{\mathrm{N}}) = \mu_3$ & \lean{BH_factor} & \lean{norm_num} \\
...
The Python script \lean{tests/verify_odd_zeta_pcf_unified.py} performs
...
\begin{align}
  G_{\mathrm{N}} &= \mu_3 = 1/2 \label{eq:ax-GN} \\
  \ell &= 1 \quad\text{(AdS radius from S-duality)} \label{eq:ax-ell} \\
  c &= 3\ell/(2G_{\mathrm{N}}) = 3 \quad\text{(Brown--Henneaux)} \label{eq:ax-c} \\
  R &= \log\golden \quad\text{(regulator)} \label{eq:ax-R} \\
  S_{\mathrm{BH}}/k_{\mathrm{B}} &= \lambda_{\log}\cdot R = \log 2 \label{eq:ax-SBH}
\end{align}
